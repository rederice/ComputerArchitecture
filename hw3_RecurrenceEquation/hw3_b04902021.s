...
func:
	addi $sp, $sp, -8
	sw 	 $ra, 4($sp)	# addr in sp(4)
	slti $t0, $s1, 2	# n < 2?
	beq  $t0, $0, recur	# n >=2 => find n/2
	addi $s4, $s2, 0 	# s4 = c
	addi $sp, $sp, 8
	jr   $ra

recur:
	sw 	 $s1, 0($sp)	# n in sp(0)
	div  $s1, $t1
	mflo $s1 			# n = n/2
	jal  func
...
itoa:
	...
	# TODO: (you should turn an integer into a pritable char with the right ASCII code to output_ascii)
		la   $t2, output_ascii    # load buffer
		li   $t1, '0'
		addi $t0, $t2, 3
		li   $t3, 10        # 10 for division

	loop:
		div  $a0, $t3       # a /= 10(3,2,1)
		mflo $a0
		mfhi $t4            # get remainder
		add  $t4, $t4, $t1  # convert to ASCII digit
		sb   $t4, ($t0)     # store it
		sub  $t0, $t0, 1    # buf ptr -1
		bne	 $t0, $t2, loop
		add  $t4, $a0, $t1
		sb   $t4, ($t0)		# (0)
	jr $ra
